...
CREATE TEMPORARY TABLE "peliculas_alquiladas" as
	(SELECT i."film_id", COUNT(r."rental_id") AS "mas_de_10_alquileres" 
	FROM "rental" AS r
...
SELECT f."film_id", f."title", an."return_date", "nombre_cliente"
FROM "alquileres_no_devueltos" AS an 
LEFT JOIN "film" AS f
ON an."film_id" = f."film_id"
WHERE "nombre_cliente" ILIKE 'Tammy Sanders';
...
WITH "peliculas_Sci-Fi" as
	(SELECT f."title", c."name", a."first_name", a."last_name", a."actor_id"
	FROM "film" AS f
...
SELECT ps."first_name", ps."last_name", ps."name" AS "categoria", ps."title"
FROM "peliculas_Sci-Fi" AS ps
WHERE EXISTS (
	SELECT 1
	FROM "actor" AS a
	WHERE a."actor_id" = ps."actor_id"
	AND ps."name" = 'Sci-Fi')
ORDER BY ps."last_name" ASC;
...
WHERE r."rental_date" >
	...
	ON i."film_id" = f."film_id"
	GROUP BY f."film_id"
	HAVING f."title" ILIKE 'Spartacus Cheaper');
...
WITH "peliculas_Music" as
	(SELECT f."title", c."name", fa."actor_id", a."first_name", a."last_name"
	FROM "film" AS f
	JOIN "film_category" AS fc
	ON f."film_id" = fc."film_id"
	JOIN "category" AS c
	...
	JOIN "actor" AS a
	ON fa."actor_id" = a."actor_id")
SELECT pm."first_name", pm."last_name", pm."name"
FROM "peliculas_Music" AS pm
WHERE NOT EXISTS (
	SELECT 1
	FROM "actor" AS a
	WHERE a."actor_id" = pm."actor_id"
	AND pm."name" = 'Music');
