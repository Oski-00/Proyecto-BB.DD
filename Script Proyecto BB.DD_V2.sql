...
SELECT f."title", f."length" AS "duración", c."name" AS "categoría"
FROM "film" AS f
...
SELECT date_trunc('day',"rental_date") AS "día", COUNT(*) AS "total_alquiler"
FROM "rental"
...
SELECT f."title", COUNT(i."film_id") AS "total_disponible"
FROM "film" AS f
...
SELECT "film_actor"."actor_id", COUNT("film_id") AS nº_películas
FROM "film_actor"
LEFT JOIN "actor"
...
SELECT "customer_id", SUM("amount") AS "total_gasto_cliente"
FROM "payment"
...
WITH "alquileres_Tammy" AS 
	...
	/* Buscamos en la columna "return_date" las fechas en donde no hay datos,
	lo que indicaría que todavía no se devuelto al película, y lo comparamos con los "film_id"
	para saber a qué películas corresponde. Obtenemos que no hay fechas de devolución para los "film_id"
	(542,914,806). Ahora buscamos los títulos de las películas que corresponden a esos "film_id" */ 
...
WITH "primer_alquiler" AS 
	...
	/* buscamos la fecha del primer alquiler de 'Spartacus Cheaper' y ahora 
	buscamos las películas que se alquilaron después de esa fecha*/
SELECT a."first_name", a."last_name", "primer_alquiler"."film_id", "primer_alquiler"."title",
	   "primer_alquiler"."rental_date"
FROM "primer_alquiler"
JOIN "film_actor" AS fa
ON "primer_alquiler"."film_id" = fa."film_id"
JOIN "actor" AS a
ON fa."actor_id" = a."actor_id"
WHERE "primer_alquiler"."rental_date" > '2005-07-08'
ORDER BY a."last_name" ASC;
...
SELECT c."customer_id", concat(c."first_name",' ', c."last_name") AS "cliente", COUNT(DISTINCT(i."film_id") ) AS "alquileres_únicos"
FROM "inventory" AS i
JOIN "rental" AS r
ON r."inventory_id" = i."inventory_id"
JOIN "customer" AS c
...
WITH "peliculas_alquiladas" AS 
	...
	JOIN "film_category" AS fc
	ON fc."film_id" = f."film_id")
SELECT c."name" AS "categoria", COUNT(*) AS "total alquileres"
